...
def predict_rf_worker(
    ...
    *,
    ...
    # We will train on labeled pixels (sparse). By default we compute features
    # on a crop around scribbles. If a cache-backed point feature getter is
    # provided, we reuse (and optionally fill) cached tile features and extract
    # features only at the labeled pixels.
    ...
    # Global coordinates for selected labeled pixels
    gy = (yy + int(y0)).astype(np.int64, copy=False)
    gx = (xx + int(x0)).astype(np.int64, copy=False)

    t0 = time()

    if get_point_features is not None:
        yield ("status", run_id, "Training: loading features for labeled pixels (tile cache)…")
        X_train = get_point_features(gy, gx, list(use_channels), img)
        F = int(X_train.shape[-1])
        y_train = (scribbles[gy, gx] - 1).astype(np.uint8, copy=False)
    else:
        # Fallback: compute features for the whole training crop
        ...
        F = int(X_crop.shape[-1])
        X_train = X_crop[yy, xx, :].reshape(-1, F)
        y_train = (scr_crop[yy, xx] - 1).astype(np.uint8, copy=False)
    ...
    def _compute_tile_features(y0, y1, x0, x1):
        ...
        if get_tile_features is not None:
            X_tile = get_tile_features(y0, y1, x0, x1, use_channels, tile_img)
        else:
